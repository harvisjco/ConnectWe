...
export const MeetingRadarBanner: React.FC<MeetingRadarBannerProps> = ({
  ...
  return (
    <aside aria-label="실시간 미팅 레이더" className="relative w-full bg-gradient-to-r from-indigo-950/80 via-slate-900/95 to-slate-900/80 border-b border-indigo-500/20 backdrop-blur-md px-3 sm:px-4 py-2 transition-all shadow-sm shadow-indigo-950/40">
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-2 text-xs">
        ...
        <div className="flex items-center gap-2 min-w-0 flex-1">
          ...
          <div className="flex items-center gap-1.5 min-w-0 truncate">
            <span className="font-semibold text-slate-100 truncate text-xs">{title}</span>
            {location && (
          ...
          {matchedPerson ? (
            <>
              <button
                ...
                className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white font-bold transition-all shadow-md shadow-indigo-600/30 active:scale-95 text-xs whitespace-nowrap group"
                title="미팅 10분 전 AI 브리핑 열기"
              >
                <FileText className="w-3.5 h-3.5 text-indigo-200 group-hover:text-white shrink-0" />
              ...
              {onOpenDebrief && (
                <button
                  ...
                  className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-rose-950/80 hover:bg-rose-900 border border-rose-500/40 text-rose-300 font-bold transition-all text-xs active:scale-95 shadow-md shadow-rose-950/40 whitespace-nowrap"
            ...
            <button
              onClick={onOpenCalendarModal}
              className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-medium transition-colors whitespace-nowrap"
            >
              <span>캘린더 관리</span>
              <ChevronRight className="w-3.5 h-3.5" />
            </button>
